...
try {
    Write-Host "Starting Jenkins VM only..." -ForegroundColor Cyan
    ...
    switch ($state) {
        ...
        "saved" {
            Write-Host "Jenkins is suspended; resuming it..." -ForegroundColor Cyan
            ...
            if ($exitCode -ne 0) {
                ...
                Write-Host "Vagrant reported an error, but Jenkins is running; continuing." -ForegroundColor Yellow
            }
        }
        "suspended" {
            Write-Host "Jenkins is suspended; resuming it..." -ForegroundColor Cyan
        ...
        "poweroff" {
            Write-Host "Starting powered-off Jenkins without provisioning..." -ForegroundColor Cyan
        ...
        default {
            throw "Cannot safely start Jenkins from state '$state'."
        }
    }
